...
export const generateMockup = async (
  ...
  try {
    ...
    if (angles.length > 0) {
      cameraInstruction = `
      CAMERA ANGLES: Generate the result showing the product from the following angles: ${angles.join(', ')}.
      ${angles.length > 1 ? "Arrange these views in a clean, professional composite grid or character-sheet style layout within the single output image." : ""}
      `;
    }
